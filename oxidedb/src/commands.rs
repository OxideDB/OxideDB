...
use crate::{
    config::{
        AnalyzePluginArgs, InstallPluginArgs, ManagePluginsArgs, PluginNameArgs, PluginSubcommands,
        RegisterSuperuserArgs, StartArgs,
    },
        ...
        for i in 0..archive.len() {
            ...
            if filename == "plugin.toml" {
                ...
                file.read_to_string(&mut content).map_err(|e| {
                    AppError::internal(format!("Failed to read plugin.toml: {}", e))
                })?;
