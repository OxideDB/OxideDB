...
impl FieldTypeDefinition for NumberFieldType {
    ...
    fn convert_value(&self, value: &JsonValue) -> Result<JsonValue, String> {
        match value {
            ...
            JsonValue::String(s) => {
                s.parse::<f64>()
                    .map(|n| JsonValue::Number(serde_json::Number::from_f64(n).unwrap_or_else(|| serde_json::Number::from(0))))
...
mod tests {
    ...
    fn test_number_conversion() {
        ...
        // String to number
        assert_eq!(field_type.convert_value(&json!("123.45")).unwrap(), json!(123.45));
