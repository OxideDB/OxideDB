...
impl FieldTypeDefinition for PhoneFieldType {
    ...
    fn validate(&self, field_name: &str, value: &JsonValue) -> Result<(), String> {
        if let Some(phone) = value.as_str() {
            ...
            if !valid_chars {
                return Err(format!("Field '{}' must be a valid phone number", field_name));
            }
            
            // Must have at least 10 digits
            let digit_count = phone.chars().filter(|c| c.is_ascii_digit()).count();
            if digit_count < 10 {
                return Err(format!("Field '{}' must contain at least 10 digits", field_name));
            }
        } else {
...
mod tests {
    ...
    fn test_phone_validation() {
        ...
        // Valid phone numbers
        assert!(field_type.validate("test", &json!("+1-555-123-4567")).is_ok());
        assert!(field_type.validate("test", &json!("(555) 123-4567")).is_ok());
    ...
    fn test_phone_conversion() {
        ...
        assert_eq!(field_type.convert_value(&json!("+1-555-123-4567")).unwrap(), json!("+15551234567"));
        assert_eq!(field_type.convert_value(&json!("(555) 123-4567")).unwrap(), json!("5551234567"));
