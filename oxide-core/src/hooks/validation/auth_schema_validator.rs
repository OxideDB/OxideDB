...
use crate::field_types::{SelectConfig, ValidationRules};
...
impl AuthSchemaValidatorHook {
    ...
    pub fn validate_auth_schema(&self, schema: &mut CollectionSchema) -> Result<(), AppError> {
        ...
        info!(
            "✅ Auth collection schema validated successfully: {}",
            schema.name
        );
        Ok(())
    }
    ...
    fn add_missing_fields(&self, schema: &mut CollectionSchema) -> Result<(), AppError> {
        ...
        if !added_fields.is_empty() {
            info!(
                "📝 Auto-added missing auth fields to collection '{}': {}",
                schema.name,
                added_fields.join(", ")
            );
        }
    ...
    fn validate_field_types(&self, schema: &CollectionSchema) -> Result<(), AppError> {
        for (field_name, required_def) in &self.config.required_fields {
            if let Some(actual_def) = schema.fields.get(field_name) {
                ...
                if !self
                    .are_field_types_compatible(&required_def.field_type, &actual_def.field_type)
                {
                    return Err(AppError::validation(
                        field_name,
                        &format!(
            ...
            if let Some(actual_def) = schema.fields.get(field_name) {
                ...
                if required_def.required && !actual_def.required {
                    return Err(AppError::validation(
                        field_name,
                        &format!(
                            "Field '{}' in auth collection '{}' must be required",
                ...
                if field_name == "email" && required_def.unique && !actual_def.unique {
                    return Err(AppError::validation(
                        field_name,
                        &format!(
                            "Field '{}' in auth collection '{}' must be unique",
                            field_name, schema.name
                        ),
                    ));
                }
    ...
    pub fn validate_field_name(
        ...
        if reserved_names.contains(&field_name) {
            return Err(AppError::validation(
                field_name,
                &format!(
                    "Field name '{}' is reserved for auth collections",
                    field_name
                ),
            ));
        }
        ...
        if auth_operation_names.contains(&field_name) {
            warn!(
                "Field name '{}' in auth collection '{}' conflicts with auth operation names",
                field_name, schema.name
            );
        }
    ...
    pub fn handle_before_record_create(
        &self,
        context: &mut crate::BeforeEventContext,
    ) -> Result<(), AppError> {
        ...
        debug!(
            "Auth schema validator hook called for record creation in collection: {}",
            context.collection
        );
        Ok(())
    }
