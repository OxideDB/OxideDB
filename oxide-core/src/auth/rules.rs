...
impl<'a> RuleEvaluator<'a> {
    ...
    pub fn evaluate(&self, rule_expr: &str) -> Result<bool, AppError> {
        debug!("🔍 RuleEvaluator: Starting evaluation of rule: '{}'", rule_expr);
        ...
        debug!("🔍 RuleEvaluator: Expanded expression: '{}'", expanded_expr);
    ...
    fn expand_request_headers(&self, expr: &str) -> Result<String, AppError> {
        let header_regex = Regex::new(r"@req\.headers\.([a-zA-Z0-9\-_]+)")
        ...
        let result = header_regex.replace_all(expr, |caps: &regex::Captures| {
            ...
            if let Some(headers) = self.context.metadata.get("headers") {
                debug!("🔍 Header expansion: Available headers: {:?}", headers);
                
                // First try exact match
                if let Some(header_value) = headers.get(header_name) {
                    if let Some(value_str) = header_value.as_str() {
                        let result = format!("'{}'", value_str.replace('\'', "\\'"));
                        debug!("🔍 Header expansion: Exact match found for '{}' = '{}'", header_name, value_str);
                        return result;
                    }
                }
                ...
                if let Some(headers_obj) = headers.as_object() {
                    ...
                    debug!("🔍 Header expansion: Trying case-insensitive match for '{}'", target_header_lower);
                    
                    for (key, value) in headers_obj {
                        debug!("🔍 Header expansion: Checking header '{}' (lowercase: '{}')", key, key.to_lowercase());
                        if key.to_lowercase() == target_header_lower {
                            if let Some(value_str) = value.as_str() {
                                let result = format!("'{}'", value_str.replace('\'', "\\'"));
                                debug!("🔍 Header expansion: Case-insensitive match found for '{}' = '{}'", key, value_str);
                                return result;
                            }
                        }
                    }
            ...
            debug!("🔍 Header expansion: Header '{}' not found, returning empty string", header_name);
            "''".to_string()
        });
        ...
        let result = user_regex.replace_all(expr, |caps: &regex::Captures| {
            ...
            if let Some(claims) = &self.context.user_claims {
                match field_name {
                    ...
                    "auth_collection" => format!("'{}'", claims.auth_collection.replace('\'', "\\'")),
                    _ => {
                        // Check custom claims
    ...
    fn expand_system_variables(&self, expr: &str) -> Result<String, AppError> {
        ...
        let hour_regex = Regex::new(r"@now\.hour")
            .map_err(|e| AppError::internal(format!("Invalid regex: {}", e)))?;
        result = hour_regex.replace_all(&result, self.current_time.hour().to_string()).to_string();

        // @now.minute - current minute (0-59)
        let minute_regex = Regex::new(r"@now\.minute")
            .map_err(|e| AppError::internal(format!("Invalid regex: {}", e)))?;
        result = minute_regex.replace_all(&result, self.current_time.minute().to_string()).to_string();

        // @now.day - current day of month (1-31)
        let day_regex = Regex::new(r"@now\.day")
            .map_err(|e| AppError::internal(format!("Invalid regex: {}", e)))?;
        result = day_regex.replace_all(&result, self.current_time.day().to_string()).to_string();

        // @now.month - current month (1-12)
        let month_regex = Regex::new(r"@now\.month")
            .map_err(|e| AppError::internal(format!("Invalid regex: {}", e)))?;
        result = month_regex.replace_all(&result, self.current_time.month().to_string()).to_string();

        // @now.year - current year
        let year_regex = Regex::new(r"@now\.year")
            .map_err(|e| AppError::internal(format!("Invalid regex: {}", e)))?;
        result = year_regex.replace_all(&result, self.current_time.year().to_string()).to_string();

        // @now.weekday - current weekday (0=Sunday, 1=Monday, etc.)
        let weekday_regex = Regex::new(r"@now\.weekday")
            .map_err(|e| AppError::internal(format!("Invalid regex: {}", e)))?;
        result = weekday_regex.replace_all(&result, self.current_time.weekday().num_days_from_sunday().to_string()).to_string();

        // @now - current timestamp (Unix timestamp) - must be processed last to avoid conflicts
        let now_regex = Regex::new(r"\b@now\b") // Word boundary to avoid matching @now.hour etc.
            .map_err(|e| AppError::internal(format!("Invalid regex: {}", e)))?;
        result = now_regex.replace_all(&result, self.current_time.timestamp().to_string()).to_string();
