...
fn main() {
    ...
    let mut users_schema = create_users_schema();
    ...
    match users_schema.prepare_data(&mut test_data) {
        Ok(()) => {
            ...
            println!("   Data with defaults: {}", serde_json::to_string_pretty(&test_data).unwrap());
        }
    ...
    match users_schema.apply_defaults(&mut with_defaults) {
        Ok(()) => {
            ...
            println!("   Original: {}", serde_json::to_string_pretty(&partial_data).unwrap());
            println!("   With defaults: {}", serde_json::to_string_pretty(&with_defaults).unwrap());
        }
...
fn create_users_schema() -> CollectionSchema {
    ...
    schema.add_field(
        "bio".to_string(),
        FieldDefinition::new(FieldType::Text)
            .with_validation(bio_validation),
    );
