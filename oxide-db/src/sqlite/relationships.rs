...
use crate::{db::SchemaAdapter, Record};
use oxide_core::{field_types::RelationshipConfig, AppError, FieldType};
...
impl SqliteDb {
    ...
    pub async fn populate_relationships(
        &self,
        collection: &str,
        records: &mut [Record],
    ) -> Result<(), AppError> {
        ...
        debug!(
            "Populating relationships for {} records in collection '{}'",
            records.len(),
            collection
        );
        ...
        let relationship_fields: Vec<(String, RelationshipConfig)> = schema
            .fields
            .iter()
        ...
        if relationship_fields.is_empty() {
            debug!(
                "No relationship fields found in collection '{}'",
                collection
            );
            return Ok(());
        }
        ...
        for (field_name, config) in relationship_fields {
            debug!(
                "Populating relationship field '{}' -> '{}'",
                field_name, config.target_collection
            );
                ...
                if let Some(field_value) = record.data.get(&field_name).cloned() {
                    let populated_value = match field_value {
                        ...
                        JsonValue::Array(ids) => {
                            ...
                            let populated_array: Vec<JsonValue> = ids
                                .iter()
                                .filter_map(|id_value| {
                                    if let JsonValue::String(id) = id_value {
        ...
        debug!(
            "Successfully populated relationships for {} records",
            records.len()
        );
        Ok(())
    }
    ...
    /// by fetching the related records from the target collections.
    pub async fn populate_specific_relationships(
        &self,
        collection: &str,
        records: &mut [Record],
        field_names: &[String],
        ...
        debug!(
            "Populating specific relationships for {} records in collection '{}', fields: {:?}",
            records.len(),
            collection,
            field_names
        );
        ...
        if relationship_fields.is_empty() {
            debug!(
                "No matching relationship fields found in collection '{}' for fields: {:?}",
        ...
        debug!(
            "Successfully populated specific relationships for {} records",
            records.len()
        );
        Ok(())
    }
    ...
    pub async fn get_related_records(
        ...
        debug!(
            "Fetching {} related records from collection '{}'",
            record_ids.len(),
            target_collection
        );
                ...
                .query_map([], |row| {
                    ...
                    for (field_name, field_def) in &target_schema_clone.fields {
                        let value = match field_def.field_type.sql_type() {
                            "TEXT" => {
                                if let Ok(Some(text_val)) =
                                    row.get::<_, Option<String>>(field_name.as_str())
                                {
                                    ...
                                    {
                                        serde_json::from_str(&text_val)
                                            .unwrap_or(JsonValue::String(text_val))
                                    } else {
                            ...
                            "REAL" => {
                                if let Ok(Some(real_val)) =
                                    row.get::<_, Option<f64>>(field_name.as_str())
                                {
                                    JsonValue::Number(
                                        serde_json::Number::from_f64(real_val)
                            ...
                            "INTEGER" => {
                                if let Ok(Some(int_val)) =
                                    row.get::<_, Option<i64>>(field_name.as_str())
                                {
                    ...
                    data_map.insert(
                        ...
                        JsonValue::Number(serde_json::Number::from(updated_at)),
                    );
                        ...
                        if let Some(ref display_field_name) = display_field {
                            if let JsonValue::Object(ref obj) = record_data {
                                if let Some(display_value) = obj.get(display_field_name) {
                                    ...
                                    simplified
                                        .insert("id".to_string(), JsonValue::String(id.clone()));
        ...
        debug!(
            "Successfully fetched {} related records",
            related_records.len()
        );
        Ok(related_records)
    }
