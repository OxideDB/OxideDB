...
impl SiteSettingsHandlers {
    ...
    pub async fn get_settings<D: Db + ?Sized>(
        ...
        Self::check_admin_permission(&user_id, "read site settings").await?;

        let settings = db.get_site_settings()
            .await
            .map_err(|e| {
        ...
        if include_health {
            match db.get_settings_health().await {
                ...
                Err(e) => {
                    ...
                    response.message = Some("Settings retrieved but health check failed".to_string());
                }
            }
        }
    ...
    pub async fn update_settings<D: Db + ?Sized>(
        ...
        Self::check_admin_permission(&user_id, "update site settings").await?;

        // Validate the update request
        Self::validate_update_request(&request)?;

        db.update_site_settings(request)
            .await
            .map_err(|e| {
    ...
    pub async fn reset_settings<D: Db + ?Sized>(
        ...
        Self::check_admin_permission(&user_id, "reset site settings").await?;

        db.reset_site_settings()
            .await
            .map_err(|e| {
    ...
    pub async fn get_settings_section<D: Db + ?Sized>(
        ...
        debug!("Getting settings section '{}' for user: {}", section, user_id);
        ...
        let section_data = db.get_settings_section(&section)
            .await
            .map_err(|e| {
                warn!("Failed to get settings section '{}': {}", section, e);
                match e {
                    AppError::NotFound { .. } => ApiError::not_found(format!("Settings section '{}' not found", section)),
                    _ => ApiError::internal(format!("Failed to retrieve settings section: {}", e)),
                }
            })?;

        info!("✅ Retrieved settings section '{}' for user: {}", section, user_id);
        Ok(section_data)
    }
    ...
    pub async fn update_settings_section<D: Db + ?Sized>(
        ...
        debug!("Updating settings section '{}' for user: {}", section, user_id);
        ...
        info!("✅ Updated settings section '{}' for user: {}", section, user_id);

        Ok(SiteSettingsResponse {
            success: true,
            message: Some(format!("Settings section '{}' updated successfully", section)),
            settings: None,
        })
    }
    ...
    pub async fn test_email_configuration<D: Db + ?Sized>(
        ...
        Self::check_admin_permission(&user_id, "test email configuration").await?;

        let test_result = db.test_email_configuration()
            .await
            .map_err(|e| {
        ...
        info!("Email configuration test result for user {}: {}", user_id, test_result);
    ...
    pub async fn get_settings_health<D: Db + ?Sized>(
        ...
        Self::check_admin_permission(&user_id, "view settings health").await?;

        let health = db.get_settings_health()
            .await
            .map_err(|e| {
                warn!("Failed to get settings health: {}", e);
    ...
    async fn check_admin_permission(user_id: &str, operation: &str) -> Result<(), ApiError> {
        // TODO: In a real implementation, this would check the user's role from the database
        // For now, we'll do a simple check - in practice, you'd integrate with your auth system
        
        // This is a placeholder implementation - you should integrate with your actual
        // user role checking system here
        debug!("Checking admin permission for user {} to {}", user_id, operation);
        
        // For now, allow all authenticated users - in production, check actual roles
        Ok(())
    }
    ...
    fn validate_update_request(request: &UpdateSiteSettingsRequest) -> Result<(), ApiError> {
        ...
        if let Some(branding) = &request.branding {
            if branding.site_title.trim().is_empty() {
                return Err(ApiError::bad_request("Site title cannot be empty".to_string()));
            }
            
            // Validate color formats if provided
            if let Some(primary_color) = &branding.primary_color {
                if !Self::is_valid_hex_color(primary_color) {
                    return Err(ApiError::bad_request("Primary color must be a valid hex color".to_string()));
                }
            }
            
            if let Some(secondary_color) = &branding.secondary_color {
                if !Self::is_valid_hex_color(secondary_color) {
                    return Err(ApiError::bad_request("Secondary color must be a valid hex color".to_string()));
                }
            }
        }
        ...
        if let Some(email) = &request.email {
            if email.enabled {
                if email.smtp_host.is_none() || email.smtp_host.as_ref().unwrap().trim().is_empty() {
                    return Err(ApiError::bad_request("SMTP host is required when email is enabled".to_string()));
                }
                
                if email.from_email.is_none() || email.from_email.as_ref().unwrap().trim().is_empty() {
                    return Err(ApiError::bad_request("From email is required when email is enabled".to_string()));
                }
                
                // Validate email format
                if let Some(from_email) = &email.from_email {
                    if !Self::is_valid_email(from_email) {
                        return Err(ApiError::bad_request("From email must be a valid email address".to_string()));
                    }
                }
            }
        }
        ...
        if let Some(general) = &request.general {
            if general.max_upload_size > 1024 * 1024 * 1024 {
                return Err(ApiError::bad_request("Maximum upload size cannot exceed 1GB".to_string()));
            }
            
            if general.api_rate_limit == 0 {
                return Err(ApiError::bad_request("API rate limit must be greater than 0".to_string()));
            }
        }
        ...
        if let Some(security) = &request.security {
            if security.password_min_length < 4 {
                return Err(ApiError::bad_request("Password minimum length must be at least 4 characters".to_string()));
            }
            
            if security.password_min_length > 128 {
                return Err(ApiError::bad_request("Password minimum length cannot exceed 128 characters".to_string()));
            }
            
            if security.session_timeout_minutes == 0 {
                return Err(ApiError::bad_request("Session timeout must be greater than 0".to_string()));
            }
        }
    ...
    fn validate_section_name(section: &str) -> Result<(), ApiError> {
        match section {
            ...
            _ => Err(ApiError::bad_request(format!("Invalid settings section: {}", section))),
        }
    }
    ...
    fn validate_section_data(section: &str, data: &Value) -> Result<(), ApiError> {
        match section {
            settings_sections::BRANDING => {
                let _branding: oxide_core::site_settings::BrandingSettings = serde_json::from_value(data.clone())
                    .map_err(|e| ApiError::bad_request(format!("Invalid branding data: {}", e)))?;
                Ok(())
            }
            settings_sections::EMAIL => {
                let _email: oxide_core::site_settings::EmailSettings = serde_json::from_value(data.clone())
                    .map_err(|e| ApiError::bad_request(format!("Invalid email data: {}", e)))?;
                Ok(())
            }
            settings_sections::SYSTEM_INFO => {
                let _system_info: oxide_core::site_settings::SystemInfoSettings = serde_json::from_value(data.clone())
                    .map_err(|e| ApiError::bad_request(format!("Invalid system info data: {}", e)))?;
                Ok(())
            }
            settings_sections::GENERAL => {
                let _general: oxide_core::site_settings::GeneralSettings = serde_json::from_value(data.clone())
                    .map_err(|e| ApiError::bad_request(format!("Invalid general settings data: {}", e)))?;
                Ok(())
            }
            settings_sections::SECURITY => {
                let _security: oxide_core::site_settings::SecuritySettings = serde_json::from_value(data.clone())
                    .map_err(|e| ApiError::bad_request(format!("Invalid security settings data: {}", e)))?;
                Ok(())
            }
            _ => Err(ApiError::bad_request(format!("Unknown settings section: {}", section))),
        }
    }
