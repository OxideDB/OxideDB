...
static PLUGIN_INSTANCE: OnceLock<Mutex<Option<Box<dyn PluginEventHandler + Send>>>> =
    OnceLock::new();
    ...
    if let Some(existing) = HTTP_HANDLER.get() {
        ...
        if let Ok(mut guard) = existing.lock() {
            *guard = Some(boxed_handler);
            ...
            crate::host::Host::log_error(
                "Failed to lock existing HTTP handler for re-initialization",
            );
        }
    } else {
        ...
        match HTTP_HANDLER.set(Mutex::new(Some(boxed_handler))) {
            ...
            Err(_) => {
                ...
                crate::host::Host::log_error(
                    "Failed to initialize HTTP handler - already set by another thread",
                );
            }
        }
    }
...
{
    let instance = HTTP_HANDLER.get().ok_or_else(|| {
        crate::host::Host::log_error(
            "HTTP_HANDLER.get() returned None - handler never initialized",
        );
    ...
    let handler = guard.as_mut().ok_or_else(|| {
        crate::host::Host::log_error(
            "HTTP handler mutex contains None - handler was initialized but then cleared",
        );
...
pub fn handle_http_request_impl() -> i32 {
    ...
    if let Some(handler_mutex) = HTTP_HANDLER.get() {
        if let Ok(guard) = handler_mutex.lock() {
            if guard.is_none() {
                crate::host::Host::log_error(
                    "HTTP_HANDLER mutex contains None - handler was not properly initialized",
                );
                let error_response =
                    crate::types::HttpResponse::error(500, "HTTP handler not initialized");
                let _ = crate::host::Host::set_http_response(&error_response);
                return 1;
            } else {
                crate::host::Host::log_info(
                    "HTTP handler mutex contains a valid handler, proceeding...",
                );
            }
        } else {
    ...
    match with_http_handler(|handler| {
        ...
        crate::host::Host::log_info(&format!(
            "HTTP request: {} {}",
